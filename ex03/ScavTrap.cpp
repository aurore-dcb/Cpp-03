...
ScavTrap::ScavTrap( void ) {
    ...
    std::cout << GREEN << "ScavTrap created" << std::endl << RESET_COLOR;
    return ;
}

ScavTrap::ScavTrap( std::string name ) : ClapTrap(name, 100, 50, 20) {

    std::cout << GREEN << "ScavTrap " << _name << " created" << std::endl << RESET_COLOR;
    return ;
}
...
ScavTrap::ScavTrap( const ScavTrap& copy ) : ClapTrap(copy) {
    ...
    std::cout << CYAN << "ScavTrap copy constructor of " << copy.getName() << " called" << std::endl << RESET_COLOR;
    *this = copy;
    return ;
}
